                  ...
                  {Object.entries(groupedLinks).map(([date, links]) =>
                    links.map((link, idx) => (
                      <div key={link._id} className="space-y-1">
                        {/* date, dot and link aligned */}
                        <div className="flex items-center">
                          <div className="w-24 text-sm font-semibold text-gray-600">
                            {idx === 0 ? date : ''}
                          </div>
                          <span className="ml-4 w-4 h-4 bg-indigo-600 rounded-full flex-shrink-0" />
                          <a
                            href={link.url}
                            target="_blank"
                            rel="noopener noreferrer"
                            className="ml-4 font-medium text-indigo-600 hover:underline break-words"
                          >
                            {new URL(link.url).hostname}
                          </a>
                        </div>
                        {/* category & source below, indented under link */}
                        <div className="text-sm text-gray-500 flex space-x-2 ml-36">
